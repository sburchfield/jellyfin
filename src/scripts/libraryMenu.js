...
function renderHeader() {
    ...
    html += '<div class="headerNavLinks hide focuscontainer-x"></div>';
    html += '</div>';
...
function updateLibraryMenu(user) {
    ...
    if (libraryMenuOptions) {
        getUserViews(apiClient, userId).then(function (result) {
            ...
            updateTopNavLinks(items);
        });
    }
}

// Netflix-style horizontal nav links in the header: Home + the user's libraries.
function updateTopNavLinks(views) {
    const nav = document.querySelector('.headerNavLinks');
    if (!nav) return;

    let html = `<a is="emby-linkbutton" class="headerNavLink" href="#/home">${globalize.translate('Home')}</a>`;
    html += views.map(function (i) {
        const href = i.url || getItemHref(i, i.CollectionType);
        return `<a is="emby-linkbutton" class="headerNavLink" data-itemid="${i.Id}" href="${href}">${escapeHtml(i.Name)}</a>`;
    }).join('');
