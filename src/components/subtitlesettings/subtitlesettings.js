...
// Netflix/Hulu-style one-tap subtitle presets. Each sets the custom appearance fields.
const SUBTITLE_PRESETS = {
    clean: { subtitleStyling: 'Custom', textSize: 'large', textWeight: 'normal', font: '', textColor: '#ffffff', dropShadow: '', textBackground: 'transparent' },
    boxed: { subtitleStyling: 'Custom', textSize: 'large', textWeight: 'normal', font: '', textColor: '#ffffff', dropShadow: 'none', textBackground: 'rgba(0,0,0,0.75)' },
    large: { subtitleStyling: 'Custom', textSize: 'extralarge', textWeight: 'bold', font: '', textColor: '#ffffff', dropShadow: '', textBackground: 'transparent' },
    yellow: { subtitleStyling: 'Custom', textSize: 'large', textWeight: 'normal', font: '', textColor: '#ffff00', dropShadow: '', textBackground: 'transparent' }
};

function setFieldValue(context, selector, value) {
...
function applySubtitlePreset(context, presetId) {
    const preset = SUBTITLE_PRESETS[presetId];
    if (!preset) return;

    // Styling must be "Custom" for the appearance fields to take effect.
    setFieldValue(context, '#selectSubtitleStyling', preset.subtitleStyling);
    setFieldValue(context, '#selectTextSize', preset.textSize);
    setFieldValue(context, '#selectTextWeight', preset.textWeight);
    setFieldValue(context, '#selectFont', preset.font);
    setFieldValue(context, '#selectDropShadow', preset.dropShadow);
    setFieldValue(context, '#inputTextBackground', preset.textBackground);
    // Color lives in different controls on TV vs desktop; set both so either reads it.
    setFieldValue(context, '#inputTextColor', preset.textColor);
    setFieldValue(context, '#selectTextColor', preset.textColor);
}
...
function embed(options, self) {
    ...
    for (const presetBtn of options.element.querySelectorAll('.subtitlePresetBtn')) {
        presetBtn.addEventListener('click', () => {
            applySubtitlePreset(options.element, presetBtn.getAttribute('data-preset'));
        });
    }
