...
import { playShuffledBlock } from './shuffleBlock';
...
export async function getCommands(options) {
    ...
    // "Shuffle Block": treat a collection like a TV network block — interleave every
    // episode from all of its shows into one continuous shuffled queue.
    if (options.shuffleBlock !== false && item.Type === 'BoxSet') {
        commands.push({
            name: 'Shuffle Block',
            id: 'shuffleblock',
            icon: 'live_tv'
    ...
    // Netflix/Hulu-style "Clear watch history": reset played state + resume position
    // for this title. For a series this cascades across every episode. Shown on all
    // playable title types (marking an already-unwatched item is a harmless no-op).
    const watchHistoryTypes = ['Movie', 'Series', 'Season', 'Episode', 'Video'];
    if (options.clearWatchHistory !== false && watchHistoryTypes.includes(item.Type)) {
        commands.push({
            name: 'Clear watch history',
            id: 'clearwatchhistory',
            icon: 'restart_alt'
        });
    }
    ...
    return new Promise(function (resolve, reject) {
        ...
        switch (id) {
            ...
            case 'clearwatchhistory':
                apiClient.markUnplayed(options.user.Id, itemId).then(function () {
                    toast('Watch history cleared');
                    getResolveFunction(resolve, id, true)();
                }, getResolveFunction(resolve, id));
                break;
            case 'shuffleblock':
                playShuffledBlock(item).then(getResolveFunction(resolve, id), getResolveFunction(resolve, id));
                break;
            case 'open':
